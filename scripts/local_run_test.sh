#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────
# run_test.sh  —  CS340400 HW1 local test runner
# Usage:
#   run_test                   → runs all testcases
#   run_test comment           → runs testcases/comment only
#   run_test debug comment     → compile and print raw output, no diff
# ─────────────────────────────────────────────────────────────
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║   CS340400 HW1 — Local Test Runner       ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════╝${NC}"
echo ""

# ── Step 1: Check scanner.l exists ───────────────────────────
...
if [ ! -f "$BUILD_DIR/scanner" ]; then
    echo -e "${RED}  ✗ 'scanner' binary not found after make.${NC}"
    exit 1
fi
...
if [ "${1:-}" = "debug" ]; then
    ...
    "$BUILD_DIR/scanner" < "$input_file" 2>&1
    echo ""
...
for input_file in "$TESTCASE_DIR"/*.txt; do
    ...
    student_out=$("$BUILD_DIR/scanner" < "$input_file" 2>/dev/null) || {
        echo -e "${RED}  CRASH ${testname} — scanner exited with error${NC}"
        ERRORS=$((ERRORS + 1))
        continue
    }
